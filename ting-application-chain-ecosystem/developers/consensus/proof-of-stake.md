...
This section aims to give a better overview of some concepts currently present in the Proof of Stake (PoS) implementation of the Ting Application Chain.

The Ting Application Chain Proof of Stake (PoS) implementation is meant to be an alternative to the existing PoA IBFT implementation, giving node operators the ability to easily choose between the two when starting a chain.
...
This contract is pre-deployed whenever a PoS mechanism Ting Application Chain chain is initialized, and is available on the address `0x0000000000000000000000000000000000001001` from block `0`.

## **Epochs**

Epochs are a concept introduced with the addition of PoS to the Ting Application Chain.
...
**Epoch Blocks** are a concept introduced in the PoS implementation of IBFT in Ting Application Chain.
...
The default size of an epoch is `100000` blocks in the Ting Application Chain.

## Contract pre-deployment <a href="#contract-pre-deployment" id="contract-pre-deployment"></a>

The Ting Application Chain _pre-deploys_ the Staking Smart Contract during **genesis generation** to the address `0x0000000000000000000000000000000000001001`.
