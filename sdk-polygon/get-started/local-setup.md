...
The below guide will instruct you on how to set up a **TingChain** network on your local machine for testing and development purposes.

The procedure differs greatly from the way you would want to set up **TingChain** network for a real use scenario on a cloud provider: [Cloud Setup](cloud-setup.md)
...
Follow this guide to issue operator commands to the newly built cluster: [How to query operator information](../working-with-node/) (the GRPC ports for the cluster we have built are `10000`/`20000`/`30000`/`40000` for each node respectively)
